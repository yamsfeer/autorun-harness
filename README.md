...
The framework supports any Anthropic-compatible API endpoint. Provider configs are stored globally at `~/.config/autorun-harness/providers/` and are the single source of truth.
...
**How provider switching works — four layers of defense:**

Each provider is defined by a **model** + **baseUrl** + **authToken** triplet. These must be switched together — changing only the model while keeping the old baseUrl will send requests to the wrong API endpoint.

Claude Code CLI loads settings with this priority (highest first):
```
1. Project .claude/settings.local.json    ← harness does NOT touch this
2. Project .claude/settings.json
3. User ~/.claude/settings.local.json     ← harness writes here (with merge)
4. User ~/.claude/settings.json           ← lowest priority
```

When the framework switches providers, it applies four layers:

1. **`process.env`** — Sets `ANTHROPIC_*` for the current process immediately.
2. **`~/.claude/settings.local.json`** — Writes the provider config to the user-level local settings, merging with any existing settings (does not overwrite the user's other preferences). This is the critical layer: priority 3 overrides priority 4 (`~/.claude/settings.json`), preventing global defaults from overriding the active provider.
3. **SDK `options.env`** — Passes the full environment (with provider overrides) to each `query()` call via the `env` option, preserving system variables via `...process.env`.
4. **Project-level conflict warning** — If a project has its own `.claude/settings.local.json` with `ANTHROPIC_*` variables (priority 1), the harness prints a warning — it does not modify project files.

> **Important:** If you have `~/.claude/settings.json` with an `env` section (e.g. for a default provider), the harness writes to `~/.claude/settings.local.json` which has higher priority. No manual changes to your global settings are needed. If a specific project has its own `.claude/settings.local.json` with `ANTHROPIC_*` settings, the harness will warn you that those take precedence.
