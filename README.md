...
**How provider switching works — three layers of defense:**
...
When the framework switches providers, it applies three layers of configuration:

1. **`process.env`** — Sets `ANTHROPIC_MODEL`, `ANTHROPIC_BASE_URL`, and `ANTHROPIC_AUTH_TOKEN` for the current process.
2. **Project-level `.claude/settings.local.json`** — Writes the provider's env config to the project directory. This is the critical layer: Claude Code CLI loads settings with **project-level > user-level** precedence, so this overrides any global defaults in `~/.claude/settings.json`.
3. **SDK `options.env`** — Passes the full environment (with provider overrides) to each `query()` call via the `env` option, preserving all system variables via `...process.env`.

> **Important:** If you have `~/.claude/settings.json` with an `env` section (e.g. for a default provider), the project-level `.claude/settings.local.json` generated by the harness will take precedence during harness execution. No manual changes to your global settings are needed.
